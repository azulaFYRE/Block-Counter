...
public class BlockRenderingServiceImpl implements BlockRenderingService {
    ...
    public void renderStandingSelection(MatrixStack matrixStack, Vec3d firstPos, BlockPos lockPos, BlockCounterModMenuConfig config) {
        ...
        if (firstPos != null) {
            ...
            this.renderLine(config, matrixStack, fixedFirst, toRender, false);
        }
    }

    public void renderClickSelection(MatrixStack matrixStack, Vec3d firstPos, BlockPos lockPos, BlockCounterModMenuConfig config) {
        ...
        if (firstPos != null) {
            ...
            if (secondPos != null) {
                this.renderLine(config, matrixStack, firstPos, secondPos, true);
            }
        }
    ...
    private void renderLine(BlockCounterModMenuConfig config, MatrixStack matrixStack, Vec3d firstPos, Vec3d secondPos, boolean isClick) {
        ...
        if (shapeService.isAxisAligned()) {
            ...
            if (!shapeService.isTwoAxis()) {
                this.renderSingleLine(config, matrixStack, firstPos, secondPos, isClick);
            } else {
                this.renderDoubleLine(config, matrixStack, firstPos, secondPos, isClick);
            }

        } else {
            this.renderFreeLine(config, matrixStack, firstPos, secondPos, isClick);
        }

    }

    private void renderSingleLine(BlockCounterModMenuConfig config, MatrixStack matrixStack, Vec3d firstPos, Vec3d secondPos, boolean isClick) {
        ...
        if (this.renderType.equals(RenderType.SOLID)) {
            RenderService.drawFilled(matrixStack, renderPos, dimensions, this.renderColor);
        } else if (this.renderType.equals(RenderType.EDGE_ONLY)) {
            // render individual blocks
            for (int b = 0; b < stopIndex; b++) {
                RenderService.drawOutlined(matrixStack, renderPos, blockDimension, this.edgeColor);
                renderPos = renderPos.add(toAdd);
            }
        } else {
            for (int b = 0; b < stopIndex; b++) {
                RenderService.drawFillAndOutlined(matrixStack, renderPos, blockDimension, this.renderColor, this.edgeColor);
                renderPos = renderPos.add(toAdd);
            }
        }
    ...
    private void renderDoubleLine(BlockCounterModMenuConfig config, MatrixStack matrixStack, Vec3d firstPos, Vec3d secondPos, boolean isClick) {
        ...
        this.renderSingleLine(config, matrixStack, firstStart, firstEnd, isClick);
        ...
        this.renderSingleLine(config, matrixStack, firstEnd, secondEnd, isClick);

    }

    // 3D Line Drawing algorithm with slight tweaks
    // https://www.geeksforgeeks.org/bresenhams-algorithm-for-3-d-line-drawing/
    private void renderFreeLine(BlockCounterModMenuConfig config, MatrixStack matrixStack, Vec3d firstPos, Vec3d secondPos, boolean isClick) {
        LineConfigService service = BlockCounterClient.getInstance().getLineConfigService();
        Vec3d offset = new Vec3d(service.getXOffset(), service.getYOffset(), service.getZOffset());
        ...
        for (Vec3d point : renderPoints) {
            ...
            if (this.renderType.equals(RenderType.SOLID)) {
                RenderService.drawFilled(matrixStack, point, blockDimension, this.renderColor);
            } else if (this.renderType.equals(RenderType.EDGE_ONLY)) {
                RenderService.drawOutlined(matrixStack, point, blockDimension, this.edgeColor);
            } else {
                RenderService.drawFillAndOutlined(matrixStack, point, blockDimension, this.renderColor, this.edgeColor);
            }
